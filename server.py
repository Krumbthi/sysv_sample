...
class Server(object):
    def __init__(self):
        self.GlobTimer = timer.GlobalTimer()
        utils.say("Oooo 'ello, I'm Mrs. Premise!")
        self.Params = utils.read_params()
        self.Semaphore = sysv_ipc.Semaphore(params["KEY"], sysv_ipc.IPC_CREX)
        self.Memory = sysv_ipc.SharedMemory(params["KEY"], sysv_ipc.IPC_CREX)

    def doSthUseful(self):
        pass
